    ...
    public static int threeSumClosest(int[] nums, int target) {
        ...
        for(int i=0;i+2<nums.length;i++){
            ...
            while(l<r){
               ...
               //int difference = Math.abs(target-(nums[i]+nums[l]+nums[r]));
               int difference = nums[i]+nums[l]+nums[r];
               if(previous >= Math.abs(target-(nums[i]+nums[l]+nums[r])) || (previous == 0 && !start)){
                  result = (nums[i]+nums[l]+nums[r]);
                  previous = Math.abs(target-(nums[i]+nums[l]+nums[r]));
                  start = true;
               }
